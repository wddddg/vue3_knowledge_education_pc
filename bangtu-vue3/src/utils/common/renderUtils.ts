...
const render = {
  ...
  renderImage: ({ text }) => {
    if (!text) {
      ...
      return h(
        Avatar,
        { shape: 'square', size: 25, style:{ width:'50px',height:'50px',"margin-right":"10px","border-radius": '50%',"text-align":'center',"line-height":"55px",flex:'none' } },
        {
          icon: () => h(Icon, { icon: 'ant-design:user-outlined', size: 25 }),
        }
      );
    }
    ...
    return h(
      ...
      avatarList.map((item) => {
        return h(Avatar, {
          ...
          shape: 'square',
          size: 25,
          style:{ marginRight: '5px',width:'50px',height:'50px',"margin-right":"10px","border-radius": '50%',"text-align":'center',"line-height":"55px",flex:'none' }
        });
      })
    );
