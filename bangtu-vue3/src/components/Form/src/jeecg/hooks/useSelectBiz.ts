...
export function useSelectBiz(getList, props) {
  ...
  //接收已选择的label
  const selectLabels = <object>inject('selectLabels', reactive({ value: [], change: false }));
  // 是否正在加载回显
  ...
  watch(
    ...
    () => {
      if (selectValues['change'] == false) {
        ...
        if(selectLabels.value&&selectLabels.value.length>0){
            let options = [];
            for(let i = 0;i<selectValues.value.length;i++){
                options.push({ label: selectLabels.value, value: selectValues.value[i] });
  ...
  async function getDataSource(params, flag) {
    ...
    if (flag) {
      ...
      records.forEach((item) => {
        options.push({ label: item[props.labelKey] || item.title, value: item[props.rowKey] });
      });
      selectOptions.value = options;
    }else{
      selectOptions.value =[]
