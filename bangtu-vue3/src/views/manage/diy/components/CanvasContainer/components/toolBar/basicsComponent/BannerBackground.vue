<template>
    <div class="content">
        <h3 class="title">背景式轮播设置</h3>
        <!-- <div class="title_text">
            <div> 1.图片建议宽度为750px，不同图片高度必须保持一致。</div>
            <div> 2.小程序自定义链接仅支持打开站内域名。</div>
        </div> -->
        <a-alert type="info" showIcon message="1.图片建议宽度为750px，不同图片高度必须保持一致。" style="margin-bottom:20px;font-size: 1px;">
            <template #description>
            <div style="font-size: 16px ;"> 2.小程序自定义链接仅支持打开站内域名。 </div>
            </template>
        </a-alert>
        <div class="item" v-for="(item, index) in props.componentContent.websiteHtmlCustomModuleLinkVOList" :key="index" @click="getIndex(index)">
        ...
        <a-button type="primary" class="addButtonIcon" @click="addListItem">添加轮播图</a-button>
    </div>
</template>
