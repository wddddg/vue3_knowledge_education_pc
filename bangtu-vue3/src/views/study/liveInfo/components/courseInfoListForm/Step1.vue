  ...
  let oneCateId =  ref('')
    ...
    const [registerCentont,{ validate:CentontData,setFieldsValue:setCentontData,resetFields:resetCentontData  } ] = useForm({
        ...
        function  resetForm  (){
            ...
            resetChargeData()
            resetCentontData()
            setClassData({isCommon:0,isClassCourse:0})
        }
        ...
        watch(
          ...
          (o,n)=>{
            ...
            if(setFormData.value.courseServerVOList &&typeof setFormData.value.courseServerVOList != 'string'){
              setFormData.value.courseServerVOList = arr.map((item,i)=>{
                return item.code
              }).join(',')
            ...
            setFormData.value['datetime']  = [setFormData.value.startTime?new Date(setFormData.value.startTime):new Date,setFormData.value.endTime?new Date(setFormData.value.endTime):new Date]
            let param = {
