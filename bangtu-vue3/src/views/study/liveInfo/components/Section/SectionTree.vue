<template>
  ...
  <a-card style="margin-top:20px">
    <BasicTree
      ...
      @expand="onExpand"
    >
      <template #title="{name,liveStatus,type}">
        <span style="color: ">{{ name }}</span>
        <span style="color: orange" v-if="liveStatus==0&&type=='node'">-(待开播)</span>
        <span style="color: green" v-else-if="liveStatus==1&&type=='node'">-(正在直播)</span>
        <span style="color: red" v-else-if="liveStatus==3&&type=='node'">-(暂时离开)</span>
        <span style="color: red" v-else-if="type=='node'">-(直播结束)</span>
      </template>
    </BasicTree>
  </a-card>
      ...
      getSectionList({courseId:recordId.value}).then(res =>{
        ...
        res.forEach((itme,i) => {
                ...
                itme.courseSectionVOList.forEach((even,c) => {
                  ...
                  node.liveStatus = even.liveStatus 
