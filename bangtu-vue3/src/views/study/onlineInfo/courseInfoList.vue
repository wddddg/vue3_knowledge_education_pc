<template>
  <div>
   ...
   <BasicTable @register="registerTable" :rowSelection="rowSelection">
    ...
    <courseUserStudy @register="registerAdminDrawe" @success="handleSuccess"></courseUserStudy>
...
<script lang="ts" name="knowledgeInfoList" setup>
  ...
  import courseUserStudy from './components/courseUserStudy/courseUserStudy.vue'
...
import { message } from 'ant-design-vue';
  ...
  async function setOnlineBatch(status) {
    if(selectedRowKeys.value.length === 0) return message.error('请选择您要操作的数据')
     await onlineBatch({ids: selectedRowKeys.value,status:status, courseType:1}, handleSuccess);
   }
   /**
    * 成功回调
    */
