...
import { saveOrUpdate } from './courseInfo.api';
...
export const columns: BasicColumn[] = [
   ...
   },
   {
    ...
    dataIndex: 'costType',
     customRender: ({ text }) => {
       const color = text == '1' ? 'green' : text == '2' ? 'blue' : text == '3' ? 'yellow': 'red';
       return render.renderTag(text==1?'免费课程':text==2?'限时免费':text==3?'会员免费':'全部收费', color);
     }
   },
   {
    title: '课程分类',
