<template>
  <div>
   ...
   <BasicTable @register="registerTable" :rowSelection="rowSelection">
    ...
    <SectionCourseInfoAdmin @register="registerAdminDrawe" @success="handleSuccess"></SectionCourseInfoAdmin>
    <Section @register="registerSectionDrawe" @success="handleSuccess"></Section>
  </div>
</template>
...
<script lang="ts" name="knowledgeInfoList" setup>
  ...
  import SectionCourseInfoAdmin from './components/courseInfoAdmin/SectionCourseInfoAdmin.vue'
...
import { message } from 'ant-design-vue';
  ...
  const [registerSectionDrawe, { openDrawer: openSectionDrawe }] = useDrawer();
  ...
  async function batchHandleDelete() {
   ...
   async function setOnlineBatch(status) {
    if(selectedRowKeys.value.length === 0) return message.error('请选择您要操作的数据')
     await onlineBatch({ids: selectedRowKeys.value,status:status, courseType: 3 }, handleSuccess);
   }
   /**
    * 成功回调
    */
