    ...
    if(recordId.value){
      getSectionList({courseId:recordId.value}).then(res =>{
        ...
        res.forEach((itme,i) => {
                ...
                section.teacherName = itme.teacherName
                section.isCommerce = itme.isCommerce
                section.isExercise = itme.isExercise
                section.courseId = itme.courseId
                section['datetime']  = [itme.startTime?new Date(itme.startTime):new Date,itme.endTime?new Date(itme.endTime):new Date]
                section.key = i + ''
                ...
                itme.courseSectionVOList.forEach((even,c) => {
                  ...
                  let status = even.currentStatus==1?'-(待上课)':even.currentStatus==2?'-(正在上课)':'-(已结束)'
                  node.title = even.name + status
                  node.name = even.name 
                  node.sort = even.sort
                  node.teacherName = even.teacherName
                  ...
                  node['datetime']  = [even.startTime?new Date(even.startTime):new Date,even.endTime?new Date(even.endTime):new Date]
                  node.type = 'node'
                  node.key = i + '-' + c
