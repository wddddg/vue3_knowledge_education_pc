...
<script lang="ts" setup>
  ...
  import {formSchema} from '../SystemCategory.data';
  import {loadTreeData, saveOrUpdateDict} from '../SystemCategory.api';
  // 获取emit
  ...
  const [registerModal, {setModalProps, closeModal}] = useModalInner(async (data) => {
    ...
    if (data?.record) {
      model = data.record;
      let param = JSON.parse(JSON.stringify(data.record))
      param.memberType = param.memberType?.split(',')
      //表单赋值
    ...
    if(!isUpdate.value||data.record.pid=='0'){
      
      updateSchema([
            { field: 'costType', ifShow: false },
            { field: 'pid', ifShow: false },
          ])
    }else{
      updateSchema([
            { field: 'costType', ifShow: true },
            { field: 'pid', ifShow: true },
          ])
    }
    //父级节点树信息
